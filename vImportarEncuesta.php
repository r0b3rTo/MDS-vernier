...
?>
  ...
  <div class="span7">
    ...
    <form id="newWeight" class="form-horizontal" method="post" action="lib/cImportarEncuesta.php?action=pesos" >
	  ...
	  ?>
		...
		<? if ($LISTA_PREGUNTA['Preg']['id_pregunta_root'][$i]==NULL) { ?> 
		  ...
		  <td class="center lsmallT" style="width: 30px" nowrap>
		    <input type="text" required name="peso_<?echo $i?>" id="peso_<?echo $i?>" data-format="d.d" value="<?if ($LISTA_PREGUNTA['Preg']['peso'][$i]!=NULL) echo $LISTA_PREGUNTA['Preg']['peso'][$i]; ?> 1" maxLength="3" placeholder="0.0" style="width: 20px;"/>
		  </td>
		  </td> 
		<? } else { ?>
		  <td class="center lsmallT" nowrap><small>&nbsp;&nbsp;&nbsp;&nbsp;&rarr;<? echo $LISTA_PREGUNTA['Preg']['titulo'][$i] ?></small></td>
		  <td class="center lsmallT" style="width: 30px" nowrap>
		    <input type="text" required name="peso_<?echo $i?>" id="peso_<?echo $i?>" data-format="d.d" value="-" maxLength="3" style="width: 20px;" readonly />
		  </td>
		  </td> 		
		<? }?>
	      </tr>
